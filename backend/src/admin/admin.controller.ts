import { Body, Controller, Delete, Get, Param, Patch, Post, Put, UseGuards } from '@nestjs/common';
import { AdminService } from './admin.service';
import { AuthGuard } from 'src/guards/auth.guard';

@Controller('admin')
export class AdminController {
    constructor(private readonly adminService: AdminService) { }

    /* 
    > Method: @Post
    > Endpoint: http://localhost:3000/admin/register
    > Input Method: JSON body
    > Purpose:
        > Creates a admin and saves it to the "database". 
        > Returns true or false if the creation is successful or not.
        > You may choose to write a function autogenerating the id. 
        > The ID may be string or number, it's up to you.
    */
    @Post('/register')
    register(@Body() body: any) {
        return this.adminService.register(body);
    }

    /* 
    > Method: @Get
    > Endpoint: http://localhost:3000/admin/display
    > Input Method: none
    > Purpose:
        > Returns a list of this.admins.
    */
    @Get('/display')
    logAllAdmins() {
        return this.adminService.logAllAdmins();
    }


    /* 
    > Method: @Get
    > Endpoint: http://localhost:3000/admin/all
    > Input Method: none
    > Purpose:
        > Returns data about all admins, passwords must NOT be returned. 
        > This function must be placed above admin/:id in the service definitions in order for it to work
    */
    @Get('/all')
    getAllAdmin() {
        return this.adminService.getAll();
    }


    /* 
    > Method: @Get
    > Endpoint: http://localhost:3000/admin/:id
    > Input Method: none
    > Purpose:
        > Returns data about a admin, password must NOT be returned
    */
    @UseGuards(AuthGuard)
    @Get('/:id')
    getAdminID(@Param('id') id: string) {
        return this.adminService.getOne(id);
    }

    /* 
    > Method: @Put
    > Endpoint: http://localhost:3000/admin/:id
    > Input Method: JSON body with no id, Param("id")
    > Purpose:
        > Using the id from the param, replaces all the values of the Admin object with the data from body. 
        > Returns true if all data was changed. 
        > If the body is missing data, return false or an error message. (Your discretion)
    */
    @Put('/:id')
    replaceValuePut(@Param('id') id: string, @Body() body: any) {
        return this.adminService.replaceValuePut(id, body);
    }

    /* 
    > Method: @Patch
    > Endpoint: http://localhost:3000/admin/:id
    > Input Method: JSON body with no id, Param("id")
    > Purpose:
        > Using the id from the param, replaces values of the Admin object with the data that exists in the JSON body. 
        > Returns true if data was changed. 
        > If the body has an invalid value return false or an error message.
        
        */
    @Patch('/:id')
    replaceValuePatch(@Param('id') id: string, @Body() body: any) {
        return this.adminService.replaceValuePatch(id, body);
    }

    /* 
    > Method: @Delete
    > Endpoint: http://localhost:3000/admin/:id
    > Input Method: Param("id")
    > Purpose:
        > Deletes the admin. 
        >If the admin does not exist return an error message. 
        >If the admin is successfully deleted, return a success message.
    */
    @Delete('/:id')
    deleteAdmin(@Param('id') id: string) {
        return this.adminService.deleteAdmin(id);
    }

    /* 
    > Method: @Get
    > Endpoint: http://localhost:3000/admin/search/:term
    > Input Method: Param("term")
    > Purpose:
        > searches for a admin that matches any term not including the password. 
        > Do not consider partial matches. 
        > You may take advantage of the String.toUpper() function to make sure the term and data is the same case when comparing
    */

    @Patch('/reset')
    resetDatabase() {
        return this.adminService.resetDatabase();
    }

}


