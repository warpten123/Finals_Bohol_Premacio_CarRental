...
export class Helper {
  ...
  static populate(): Map<string, Admin> {
    var result: Map<string, Admin> = new Map<string, Admin>();
    try {
      var admins = [
        new Admin('Leanne Graham', 18, 'sincere@april.biz', 'LG_123456'),
        new Admin('Ervin Howell', 21, 'shanna@melissa.tv', 'EH_123123'),
        new Admin('Nathan Plains', 25, 'nathan@yesenia.net', 'NP_812415'),
        new Admin('Patricia Lebsack', 18, 'patty@kory.org', 'PL_12345'),
      ];
      admins.forEach((admin) => {
        result.set(admin.id, admin);
      });
      return result;
    } catch (error) {
