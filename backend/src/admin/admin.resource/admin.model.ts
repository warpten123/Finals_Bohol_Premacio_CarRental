...
export class Admin {
  ...
  static async retrieve(id: string): Promise<Admin> {
    try {
      var DB = admin.firestore();
      var result = await DB.collection("admins").doc(id).get();
      if (result.exists) {
      ...
      console.log("Admin.retrieve error");
      console.log(error.message);
      return null;
    }
  }

  static async retrieveViaEmail(email: string): Promise<Admin> {
    var DB = admin.firestore();
    var adminResults = await DB.collection("admins")
      .where("email", "==", email)
      .get();
    if (adminResults.empty) return null;
  ...
  async commit(hidePassword: boolean = true): Promise<CRUDReturn> {
    try {
      var DB = admin.firestore();
      var result = await DB.collection("admins").doc(this.id).set(this.toJson(hidePassword));
      return {
        success: true,
        data: this.toJson(true),
      };
    } catch (error) {
      console.log("Admin.committ error message");
      console.log(error.message);
  ...
  replaceValues(body: any): boolean {
    try {
      ...
      console.log("Admin.replaceValues error");
      console.log(error.message);
      return false;
    }
  }
