...
export class Helper {
  ...
  static populate(): Map<string, Cars> {
    var result: Map<string, Cars> = new Map<string, Cars>();
    try {
      var cars = [
        new Cars('Leanne Graham', 18, 'sincere@april.biz', 'LG_123456'),
        new Cars('Ervin Howell', 21, 'shanna@melissa.tv', 'EH_123123'),
        new Cars('Nathan Plains', 25, 'nathan@yesenia.net', 'NP_812415'),
        new Cars('Patricia Lebsack', 18, 'patty@kory.org', 'PL_12345'),
      ];
      cars.forEach((car) => {
        result.set(car.id, car);
      });
      return result;
    } catch (error) {
